...
* 
* @version 1.02	2019-11-16 PETS WITH ROLES
* 
*/
	...
	protected void AddTheEntryToTheBottom() {
		...
		case "life cycle": 
			try{  
				   ...
				   //Giving password to allow the connection to the database server
				   Connection connection=DriverManager.getConnection("jdbc:mysql://localhost/db_w13_petswithroles?autoReconnect=true&useSSL=false", "root" , "akshay567");  
				   System.out.println("ok");
				   //After connecting linking the statement to execute
				   Statement statement = connection.createStatement();
				   //Storing the attributes in the string and inserting them in statement
				   String a, b;
				   a = theItemName;
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute(" INSERT INTO lifecycles (lifeCycle_Name,lifeCycle_Description) VALUES ( '" +a+ "', '" +b+ "') ");
				   System.out.println("Data Inserted in LifeCycle Tab");
		...
		case "step":
			try{  
				   ...
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute(" INSERT INTO lifecycles (step_Name,step_Description) VALUES ( '" +a+ "', '" +b+ "') ");
				   System.out.println("Data Inserted in Step Tab");
				   connection.close();  //Ends the connection
			}
			catch(Exception e){ System.out.println(e);} 
			break;
		...
		case "task":
			try{  
				   ...
				   a = theItemName;
				   b = theItemDescription;
				   c = TaskSpecific.theUsedArtifact.getValue();
				   d = TaskSpecific.theProducedArtifact.getValue();
				   e = TaskSpecific.chooseUsedEffortCategory.getValue();
				   //Executes the SQL statement by connecting through the password
				   statement.execute("Insert Into tasks(task_Name, task_Description, UsedArtifactNames, ProducedArtifactNames,ArtifactEffortCategory) values('"+a+"', '"+b+"', '"+c+"', '"+d+"', '"+e+"') ");
				   System.out.println("Data Inserted in Task Tab");
				   connection.close();  //Ends the connection
			}
			catch(Exception e){ System.out.println(e);} 
			break;
		...
		case "role":
			try{  
				   ...
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute("INSERT INTO roles (role_Name,role_Description) VALUES ( '" +a+ "', '" +b+ "' )");
				   System.out.println("Data Inserted in Role Tab");
				   connection.close();  //Ends the connection
			}
			catch(Exception e){ System.out.println(e);} 
			break;
		...
		case "condition":
			try{  
				   Class.forName("com.mysql.cj.jdbc.Driver");  
				   //Giving password to allow the connection to the database server
				   Connection connection=DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/db_w13_petswithroles?autoReconnect=true&useSSL=false", "root" , "akshay567"); 		     
				   System.out.println("ok");
				   //After connecting linking the statement to execute
				   Statement statement = connection.createStatement();
				 //Storing the attributes in the string and inserting them in statement
				   ...
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute("INSERT INTO conditions (condition_Name,condition_Description) VALUES ( '" +a+ "', '" +b+ "' )");
			       System.out.println("Data Inserted in Condition Tab");
			       connection.close();  //Ends the connection
			}
			catch(Exception e){ System.out.println(e);} 
			break;
		...
		case "effort category":
			try{  
				   ...
				   //After connecting linking the statement to execute
				   Statement statement = connection.createStatement();
				           if(EffortCategorySpecific.theChoice.getValue()=="Artifacts") 
				           {  
				        	//Storing the attributes in the string and inserting them in statement
							   String a, b, c, d;
							   a = theItemName;
							   b = theItemDescription; 
							   c = EffortCategorySpecific.theOption.getValue();
							   d = EffortCategorySpecific.theChoice.getValue();
				        	//Executes the SQL statement by connecting through the password   
				        	statement.execute("INSERT INTO effortcategories (effortCategory_Name,effortCategory_Description,effortCategory_Kind,ArtifactForThisEffortCategory) VALUES ( '"+a+ "', '" +b+ "', '"+c+ "', '"+d+"' )");
				            System.out.println("Data Inserted in EffortCategory Tab");
				            connection.close();   //Ends the connection
				           }
				           if (EffortCategorySpecific.theChoice.getValue()=="Plans") 
				           {  
				        	//Storing the attributes in the string and inserting them in statement
							   String a, b, c, d;
							   a = theItemName;
							   ...
							   d = EffortCategorySpecific.theChoice.getValue();
				        	//Executes the SQL statement by connecting through the password   
				        	statement.execute("INSERT INTO effortcategories (effortCategory_Name,effortCategory_Description,effortCategory_Kind,PlanForThisEffortCategory) VALUES (  '"+a+ "', '" +b+ "', '"+c+ "', '"+d+"' )");
				            System.out.println("Data Inserted in EffortCategory Tab");
				            connection.close();   //Ends the connection
				           } 
				           if(EffortCategorySpecific.theChoice.getValue()=="Interruptions") 
				           {
				        	//Storing the attributes in the string and inserting them in statement
							   String a, b, c, d;
							   a = theItemName;
							   ...
							   d = EffortCategorySpecific.theChoice.getValue();
				        	//Executes the SQL statement by connecting through the password	   
				        	statement.execute("INSERT INTO effortcategories (effortCategory_Name,effortCategory_Description,effortCategory_Kind,InterruptionsForThisEffortCategory) VALUES (  '"+a+ "', '" +b+ "', '"+c+ "', '"+d+"' ");
				            System.out.println("Data Inserted in EffortCategory Tab");
				            connection.close();   //Ends the connection
				           }
				           if(EffortCategorySpecific.theChoice.getValue()=="Defects") 
				           {   
				        	//Storing the attributes in the string and inserting them in statement
							   String a, b, c, d;
							   a = theItemName;
							   ...
							   d = EffortCategorySpecific.theChoice.getValue();
				        	//Executes the SQL statement by connecting through the password   
				        	statement.execute("INSERT INTO effortcategories (effortCategory_Name,effortCategory_Description,effortCategory_Kind,DefectsForThisEffortCategory) VALUES (  '"+a+ "', '" +b+ "', '"+c+ "', '"+d+"' )");
				            System.out.println("Data Inserted in EffortCategory Tab");
				            connection.close();   //Ends the connection
				           }
				           if(EffortCategorySpecific.theChoice.getValue()=="Other") 
				           {  
		...
		case "artifact":
			try{  
				   ...
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute("INSERT INTO artifacts (artifact_Name,artifact_Description) VALUES ( '" +a+ "', '" +b+ "' )");
                   System.out.println("Data Inserted in Artifact Tab");
                   connection.close();  //Ends the connection
			}
			catch(Exception e){ System.out.println(e);} 
			break;
		...
		case "plan":
			try{  
				   ...
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute("INSERT INTO plans (plan_Name,plan_Description) VALUES ( '" +a+ "', '" +b+ "' )");
                   System.out.println("Data Inserted in Plan Tab");
                   connection.close();  //Ends the connection
			}
			catch(Exception e){ System.out.println(e);} 
			break;
		...
		case "interruption":
			try{  
				   ...
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute("INSERT INTO interruptions (interruption_Name,interruption_Description) VALUES ( '" +a+ "', '" +b+ "' )");
                   System.out.println("Data Inserted in Interruption Tab");
                   connection.close();  //Ends the connection 
			}
			catch(Exception e){ System.out.println(e);} 
			break;
		...
		case "defect":
			try{  
				   ...
				   b = theItemDescription;
				   //Executes the SQL statement by connecting through the password
				   statement.execute("INSERT INTO defects (defect_Name,defect_Description) VALUES ( '" +a+ "', '" +b+ "' )");
				   System.out.println("Data Inserted in Defect Tab");
				   connection.close();  //Ends the connection
			}
			catch(Exception e){ System.out.println(e);} 
			break;
